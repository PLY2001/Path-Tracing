...
Texture::Texture(const std::string& path, const std::string texturemode) :RendererID(0), FilePath(path), LocalBuffer(nullptr), Width(0), Height(0), BBP(0)
{
	HDRLoaderResult hdrRes;
	bool r = HDRLoader::load(path.c_str(), hdrRes);
	...
	if(texturemode == "HDRMap")
	{
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB32F, hdrRes.width, hdrRes.height, 0, GL_RGB, GL_FLOAT, hdrRes.cols);
	}
	else if (texturemode != "HDRMap")
	{
		// hdr 重要性采样 cache
...
// 计算 HDR 贴图相关缓存信息
float* Texture::calculateHdrCache(float* HDR, int width, int height) {
